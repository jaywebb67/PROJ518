...
save_path    = os.path.join("GANs", "capsGAN", "capsGAN-checkpoint",f"{g}")
if not os.path.exists(save_path):
    os.makedirs(save_path)

# Create folder to hold result
result_folder = os.path.join("GANs", "capsGAN", "capsGAN-result",f"{g}")
...
generator = CapsGANGenerator(latent_dim=128).to(device)
...
for epoch in range(1, num_epochs+1):
    ...
    for latents, real_imgs,_ in train_dataloader:
        ...
        loss_real = margin_loss(d_real, true_label)
        fake_imgs = generator(latents).detach()
        d_fake    = discriminator(fake_imgs)
        loss_fake = margin_loss(d_fake, false_label[:B])
        ...
        lossG     = margin_loss(d_out, true_label[:B])
    ...
    if epoch % 5 == 0:
        with torch.no_grad():
            ...
            ax.axis("off")
            ax.set_title(
                f"CapsGAN Epoch {epoch}/{num_epochs}\n"
                f"D_loss: {avg_D:.4f}, G_loss: {avg_G:.4f}",
                fontsize=12,
                color="white",
    ...
    if epoch % 25 == 0 or avg_G < min(G_loss_history):
        ...
        }, os.path.join(save_path,f"capsGANcheckpoint_epoch{epoch:03d}.pth"))
